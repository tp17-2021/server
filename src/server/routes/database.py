...
async def get_keys(collection_name: str) -> list:
    """
...
@router.get("/schema", response_model=schemas.Collections)
async def schema() -> list:
...
@router.post("/import-data", response_model=schemas.Message)
...
async def choose_candidates(candidates) -> list:
    """ Randomly choose from 1 to 4 candidates """

    candidates = random.sample(candidates, random.randint(1,5))
    return candidates


@router.post("/seed-data", response_model=schemas.Message)
async def seed_data(number_of_votes: int) -> dict:
    """ Import data from database and seed votes """
...
async def seed_votes(number_of_votes: int) -> dict:
    """ Seed votes """
...
@router.post("/commission-paper", response_model=schemas.Message)
async def upload_commission_paper(request: schemas.CommissionPaperToBeDecrypted) -> dict:
    """ Upload commission paper to database """
    ...
    content = {
        "status": "success",
        "message": "Commission paper was inserted into database"
    }
    return content
